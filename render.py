    ...
    def generateTableData(self, path, rows):
        ...
        for cmp_obj in self.order_cmp_keys:
            ...
            if cmp_obj not in self.order_base_keys:
                ...
                content = (method, str(cmp_thread), str(base_index), str(cmp_index), str(base_time), str(cmp_time), str(diff), str(rate), str(base_call_times), str(cmp_call_times), str(rank_change))
                data.append(content)
        if add_flag == 1:
        ...
        for base_obj in self.order_base_keys:
            ...
            try:
                ...
                rate = "error"
            content = (method, str(base_thread), str(base_index), str(cmp_index), str(base_time), str(cmp_time), str(diff), str(rate), str(base_call_times), str(cmp_call_times), str(rank_change))
            data.append(content)
        self.generateTable(path,rows,data)
        print "self.base_cost-self.cmp_cost:\n",self.base_cost-self.cmp_cost
        print
        print "self.base_method_thread:\n",self.base_method_thread
        print
        print "self.cmp_method_thread:\n",self.cmp_method_thread